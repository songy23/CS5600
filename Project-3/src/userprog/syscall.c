...
#define USER_ADDR_BOTTOM 0x08048000 /* Refer to SysV-i386. */
#define USER_BSS_BOTTOM 0x08052000  /* Estimated value. */
...
/* Given a file descriptor and a list of fd_node, 
...
sys_mmap (struct intr_frame *f) {
  // syscall 2, two arguments: fd, addr
  int *esp = (int *)f->esp;    // esp address 
  validate_pointer (esp + 1);
  validate_pointer (esp + 2);
  int fd = *(esp + 1);         // file descriptor
  void *addr = *(esp + 2);     // starting address to map the file
  struct thread *cur = thread_current ();
  struct list* fd_list = &(cur->fd_list);
  struct fd_list_node *node = find_fd_in_list (fd_list, fd);
  ...
  /* use the file_reopen function to obtain a separate and independent 
     reference to the file for each of its mappings. */
  struct file *file = file_reopen (node->file);
  int file_size = file_length (file);
  int page_index = 0;
  
  /* map the file, page by page */
  while (file_size > 0) {
    struct supplemental_page *new_sp_page = 
    ...
    new_sp_page->upage = addr;
    new_sp_page->file = file;
    new_sp_page->offset = page_index * (int) PGSIZE;
    new_sp_page->mapid = mapid;
    new_sp_page->writable = true;
    new_sp_page->swap_index = -1;
    new_sp_page->loaded = false;
    if (file_size >= PGSIZE) {  /* Normal "full" pages. */
      new_sp_page->read_bytes = PGSIZE;
      new_sp_page->zero_bytes = 0;
    } else {  
      /* Set the "stick out" bytes in the last page to zero when faulted. */
      new_sp_page->read_bytes = file_size;
      new_sp_page->zero_bytes = PGSIZE - file_size;
    }
    lock_acquire (&(cur->sp_table_lock));
    list_push_back (&(cur->sup_page_table), &(new_sp_page->elem));
...
sys_munmap (struct intr_frame *f) {
  ...
  struct thread *cur = thread_current ();
  struct list_elem *e;
  struct list *sp_table = &(cur->sup_page_table);
  lock_acquire (&(cur->sp_table_lock));
  for (e = list_begin (sp_table); e != list_end (sp_table); ) {
    struct supplemental_page *page = 
...
validate_pointer (const void *address) {
  if (!is_user_vaddr (address) || address < USER_ADDR_BOTTOM ||
