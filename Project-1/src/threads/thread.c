...
/* Compare the priority of two input threads. If the priority of thread a is higher,
...
/* Check the status of sleeping thread at each tick. If the wake_time
   of a thread becomes 0, then wake it up. */
void
//- Instead of counting down, you should have just set wake_time to 
//- the time when thread will wake up so you don't have to decrement the
//- thread everytime. 
sleeping_thread_check (struct thread *t, void *aux UNUSED)
{
  if (t->status == THREAD_SLEEPING && t->wake_time > 0)
  {
    t->wake_time--;
    if (t->wake_time == 0)
    {
...
/* Wake up a thread and put it into the ready list. */
void
...
thread_set_priority (int new_priority) 
{
  ...
  //- You shouldn't check if the new_priority is greater than the current. 
  //- This prevents threads from having their prioity lowered. `
